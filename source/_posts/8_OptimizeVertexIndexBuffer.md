...
需要注意的是**VkIndexType**我们需要封装进来，因为并不是索引的设备都支持32位的索引类型的。低端设备会支持**VK_INDEX_TYPE_UINT16**，无符号的16位索引最大可以支持到65535个索引，也就是65535/3个三角形。IndexBuffer封装完成了之后，创建的代码我从前面的Demo里面拷贝过来。如下：
