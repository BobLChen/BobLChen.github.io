---
title: 23_OptimizeRenderTarget
date: 2019-10-13 23:11:11
tags:
- Vulkan
- 3D
- RenderTarget
...
上个Demo里面，我们创建附件的时候写了太多代码，也进行许多配置，为了简化后续的使用，在这个Demo里面进行简单的封装。

<!-- more -->

![23_OptimizeRenderTarget](https://raw.githubusercontent.com/BobLChen/VulkanDemos/master/preview/22_RenderTarget.gif)

## DVKRenderPassInfo

RenderTarget输出的数据是多样性的，主要体现在Color Attachment上，Depth数据确是最多只有一个。这些附件在使用的时候是配置一下它们的行为的，例如希望在使用之前清除掉附件的数据，这个跟我们写代码希望初始化一样；当然某些情况下更希望复用数据不进行任何操作等。

为了满足上述的一些要求，我特意准备了一个DVKRenderPassInfo来容纳这些信息。代码如下：
```c++
...
有了Colors以及DepthStencil之后，还不能创建出RenderTarget，毕竟Colors是一个数组，它里面的元素具体的使用情况并不清楚。这个的情况其实跟DescriptorLayout很像，所以这里我也设想一个RenderTargetLayout出来，代码如下：
...
上个Demo里面，可以发现最终使用的是RenderPass和FrameBuffer，这两个东西也需要简单封装一下，组成我们的RenderTarget的元素其实就是这两个，为了使用方便把这两个的创建、维护、管理给封装了一下而已。
...
有了以上的封装之后，就可以着手将它们组合起来。组合的过程其实非常简单，主要的问题在于Image的Layout更改。在之前的Demo里面，Layout的更改是靠**VkSubpassDependency**来进行转换的，配置好VkSubpassDependency之后，会隐式的对Image的Layout进行转换。我们封装了RenderTarget之后就需要显示的进行转换。
...
我将它们的实现也贴了出来，篇幅可能会比较长，主要的原因还是实现稍微有点儿复杂以及绕。想看具体的实现可以去github上面看看。主要的思想还是自动维护创建RenderTarget所需要的元素。

## 其它

这个Demo里面其实没有太多需要说明的东西，做的主要工作就是简单封装了一个RenderTarget方面后续使用而已。
