...
在[14_TextureArray](https://github.com/BobLChen/VulkanDemos/tree/master/examples/14_TextureArray)这个Demo里面，我们了解到如何通过TextureArray技术来实现一个非常简易的地形。如果我们从另外一个维度来想象TextureArray，把**Array**想象成一个新的维度，那之前的TextureArray也就是一个Textue3D的功能。Vulkan API也提供了Texture3D的功能，在创建Texture3D的时候只需要把深度指定上即可。
...
虽然使用的情况比较少，但是还是有必要来学习一下，因为在某些情况下又是特别有用。如果对摄影比较感兴趣的朋友，可能对LUT这个技术有一定的了解，这个技术在摄影行业里面比较常用。既然在摄影里面比较常用，那么在渲染中一样也是比较常用的技术，毕竟渲染就是为了模拟现实。LUT说白了就是一个**色调映射**技术，将一个颜色值映射到另一个颜色值上。常规的颜色值由**R、G、B**三个分量组成，每个分量可以由[0, 255]表示，也就是**RGB**可以表示**256x256x256=16777216**，也就是那些手机厂商最喜欢的文案，号称它们的手机支持**1600万**色的原因。

### 色调映射方案

#### 1、直接映射
既然想要实现色调映射，那么就必须要解决**16777216**种颜色的映射。一种方案是简单粗暴的直接进行映射，用一个Map来进行映射，存储16777216对键值数据。我们来计算一下需要的最小内存和显存数据：**16777216x4/1024/=64MB**，由此可以计算出来至少需要64MB的数据。大家可以仔细思考一下，按照我们目前掌握的Demo知识来看，除了Texture可以存储这么庞大的数据以外，并没有其它方案可以解决这个64MB的问题。

### 2、Texture2D
前面提到，根据目前掌握的知识里面只有Texture可以解决这个问题。我们可以通过一张Texture2D来解决这个问题，对Texture2D进行分块，一小块的尺寸为**256x256**，一行放置**16**块，放置**16**列。这样就只需要一张**4096x4096**的图片完成色调映射的功能。即小块**256x256**映射**RG**数据，总的块数量**16x16**映射**B**数据。虽然这样可行，但是大家会发现一个问题，映射起来非常麻烦，得要根据UV值解算出来每一个块的坐标，然后进行映射。

### 3、Texture3D
前面两种方案要么行不通要么麻烦，利用Texture3D就可以简单的实现这个功能。Texture3D就是三个维度**X、Y、Z**也可以理解为**R、G、B**，在Shader里面进行采样的时候，给定的就是三个维度的坐标参数。那么在Shader里面就可以通过texture传入**color**值进行采样得到对应存储到Texture3D中的颜色值。

### 准备Texture3D数据
Texture3D数据不能直接从磁盘里面获得了，得要我们自己准备。因为常规的文件格式都是二维的，但是需要的是三维的数据，在维度上面少了一个维度。

既然要做色调映射这个功能，在准备数据的时候就要准备**RGB**值对应的映射值。我们现在来做一个怀旧风格的色调映射。由于**RGB**值的范围是[0, 255]，因此把尺寸设置为**256**，刚好覆盖**RGB**的范围。然后整三个for循环，即可把所有的颜色值的组合都遍历完，在遍历的时候进行色调的映射，代码如下：
```c++
int32 lutSize  = 256;
...
### 创建Texture3D
准备好了数据之后，现在来看一下如何创建Textuer3D，创建过程其实跟之前创建Texture2D差异不大，只是多了一个维度。代码如下：
```c++
...
```
可以看出，只是之前在创建时**depth**设置为了1，现在**depth**设置为了第三个维度的值。
...
```
最终的效果就是首页预览图的效果。当然这里有同学会说怀旧的效果完全可以不同这么麻烦，只需要在Fragment Shader里面用数学公式就可以实现。这里只是为了演示这个功能，做了一个怀旧的效果。真正的做法应该是，截一张非常满意的场景图，然后交给美术放到PS里面进行任意的色调修改，最后导出一张色调映射图，再进行使用。在这种情况下，色调的映射是按照美术的想法来完成的，绝不是单纯的数学公式可以能够做到的。